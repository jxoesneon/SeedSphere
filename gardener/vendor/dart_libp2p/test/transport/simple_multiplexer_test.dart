...
void main() {
  ...
  group('Simple Multiplexer Tests', () {
    test('basic multiplexer stream creation', () async {
      ...
      final localPeer = PeerId.fromPublicKey(
        (await generateEd25519KeyPair()).publicKey,
      );
      final remotePeer = PeerId.fromPublicKey(
        (await generateEd25519KeyPair()).publicKey,
      );
      ...
      final securedConn = await mockSecurityProtocol.secureOutbound(
        mockTransportConn,
      );
      ...
      final muxedConn =
          await multiplexerFactory.newConnOnTransport(
                securedConn,
                false,
                mux.NullScope(),
              )
    ...
    test('multiplexer with muxed connection', () async {
      ...
      final muxedConn = mux.StreamlinedMockMuxedConn(
        securedConn,
        false,
        multiplexer,
      );
