...
async function fetchStreams(type, id, timeoutMs = 6000) {
  ...
  try {
    ...
    const detailLinks = Array.from(new Set(Array.from(html.matchAll(/href=\"(\/file\/[a-z0-9]\/[a-z0-9\-]+\/)\"/gi)).map(m => m[1]))).slice(0, 8)
    ...
    const streams = magnets.map((u) => ({
      ...
      url: u,
      behaviorHints: {},
      // Best-effort extras (not available from MagnetDL HTML without heavy parsing)
      description: '',
      seeds: null,
      leechers: null,
      size: null,
