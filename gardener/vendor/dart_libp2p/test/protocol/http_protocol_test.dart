  ...
  group('HTTP-like Protocol Tests', () {
    ...
    test('basic HTTP GET request/response', () async {
      ...
      try {
        ...
        await clientHost.peerStore.addrBook.addAddrs(serverHost.id, [
          actualListenAddr,
        ...
        final response2 = await httpClient.getRequest(
          serverHost.id,
          '/users/123',
        );
        ...
        final response3 = await httpClient.getRequest(
          serverHost.id,
          '/nonexistent',
        );
    ...
    test('HTTP POST with JSON data', () async {
      ...
      try {
        ...
        final userData = {'name': 'Alice Smith', 'email': 'alice@example.com'};

        final response1 = await httpClient.postJson(
          serverHost.id,
          '/users',
          userData,
        );
    ...
    test('HTTP protocol with connection reuse', () async {
      ...
      try {
        ...
        final response1 = await httpClient.getRequest(
          serverHost.id,
          '/api/status',
        );
        expect(response1.status, equals(HttpStatus.ok));
        ...
        final response3 = await httpClient.postJson(
          serverHost.id,
          '/api/submit',
          submitData,
        );
    ...
    test('HTTP error handling and status codes', () async {
      ...
      try {
        ...
        final response5 = await httpClient.getRequest(
          serverHost.id,
          '/nonexistent',
        );
        ...
        // Missing email field
        final response7 = await httpClient.postJson(
          serverHost.id,
          '/validate',
          {'name': 'John'},
        );
        ...
        final response8 = await httpClient.postJson(
          serverHost.id,
          '/validate',
          validData,
        );
...
Future<BasicHost> createTestHost({
  ...
  final host = await BasicHost.create(network: swarm, config: config);
