...
class MultistreamMuxer implements ProtocolSwitch {
  ...
  MultistreamMuxer({MultistreamConfig? config})
  ...
  Future<void> addHandlerWithFunc(
    ...
    await _handlerLock.synchronized(() async {
      ...
      _handlers.add(
        Handler(matchFunc: match, handle: handler, addName: protocol),
      );
    });
  }
  ...
  Future<(ProtocolID, HandlerFunc)> negotiate(P2PStream<dynamic> stream) async {
    try {
      ...
      _log.severe(
        '[multistreamMuxer - negotiate] Error during negotiation: $e',
      );
      await stream.reset();
      rethrow;
    }
      ...
      if (lengthDecoded) {
        if (bytesRead >= length + varintBytesRead) {
          ...
          return Uint8List.fromList(
            fullMessage.sublist(varintBytesRead, length + varintBytesRead - 1),
          );
        }
      }
    }
