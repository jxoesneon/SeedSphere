...
class RelayFinder {
  ...
  RelayFinder(this.host, this.upgrader, this.config)
    ...
    _scheduledWorkTimes = ScheduledWorkTimes(
      nextAllowedCallToPeerSource: config.clock.now().subtract(
  ...
  void _scheduleNextWork(StreamController<void> peerSourceRateLimiter) {
    _workTimer?.cancel();
    final nextRunTime = _runScheduledWork(
      config.clock.now(),
      peerSourceRateLimiter,
    );
  ...
  DateTime _runScheduledWork(
    ...
    if (now.isAfter(_scheduledWorkTimes.nextAllowedCallToPeerSource)) {
      if (!peerSourceRateLimiter.isClosed &&
        ...
        if (_scheduledWorkTimes.nextAllowedCallToPeerSource.isBefore(
          nextGlobalTime,
        )) {
  ...
  void _findNodes(
    ...
    await for (var _ in peerSourceRateLimiter.takeUntil(stopSignal)) {
      ...
      final int numCandidates = await _candidateMx.synchronized(
        () => _candidates.length,
      );
      if (numCandidates < config.minCandidates) {
        metricsTracer.candidateLoopState(
          CandidateLoopState.peerSourceRateLimited,
        );
        currentPeerStream = _peerSource(config.maxCandidates);

        currentPeerSubscription = currentPeerStream?.listen(
          (addrInfo) async {
            final bool isOnBackoff = await _candidateMx.synchronized(
              () => _backoff.containsKey(addrInfo.id),
            );
            if (isOnBackoff) return;
            final int currentNumCandidates = await _candidateMx.synchronized(
              () => _candidates.length,
            );
  ...
  Future<bool> _handleNewNode(AddrInfo addrInfo) async {
    final bool isRelayInUse = await _relayMx.synchronized(
      () => _isUsingRelay(addrInfo.id),
    );
    if (isRelayInUse) return false;

    try {
      final supportsV2 = await _tryNode(
        addrInfo,
  ...
  Future<bool> _tryNode(AddrInfo addrInfo) async {
    ...
    final supportedProtocols = await host.peerStore.protoBook.supportsProtocols(
      addrInfo.id,
  ...
  void _handleNewCandidates(Stream<void> stopSignal) async {
    ...
    _maybeConnectToRelayTriggerController.stream.takeUntil(stopSignal).listen((
      _,
    ) {
  ...
  Future<void> _maybeConnectToRelay() async {
    ...
    final List<Candidate> selectedCandidates = await _candidateMx.synchronized(
      () => _selectCandidates(),
    );

    for (var cand in selectedCandidates) {
      final PeerId id = cand.addrInfo.id;
      final bool usingThisRelay = await _relayMx.synchronized(
        () => _isUsingRelay(id),
      );
      if (usingThisRelay) {
      ...
      try {
        final rsvp = await _connectToRelay(
          cand,
  ...
  void _cleanupDisconnectedPeers(Stream<void> stopSignal) async {
    final eventBusSubscription = host.eventBus.subscribe(
      EvtPeerConnectednessChanged,
    );
    StreamSubscription? streamSub;

    streamSub = eventBusSubscription.stream.takeUntil(stopSignal).listen((
      event,
    ) {
  ...
  Future<List<MultiAddr>> getRelayAddrs(
    ...
    return _relayMx.synchronized<List<MultiAddr>>(() async {
      ...
      final List<Future<void>> addrProcessingFutures = [];
      ...
      _relays.forEach((peerId, reservation) {
        ...
        final future = host.peerStore.addrBook.addrs(peerId).then((
          relayPeerAddrs,
        ) {
          final cleanedRelayPeerAddrs = address_utils.cleanupAddressSet(
            relayPeerAddrs,
          );
      ...
      await Future.wait(
        addrProcessingFutures,
      ); // Wait for all async addr processing
