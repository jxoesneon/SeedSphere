  ...
  group('Yamux Multiplexing', () {
    ...
    tearDown(() async {
      ...
      try {
        ...
        if (!conn1.isClosed) {
          await conn1.close().catchError(
            (e) => print('Error closing conn1: $e'),
          );
        }
        if (!conn2.isClosed) {
          await conn2.close().catchError(
            (e) => print('Error closing conn2: $e'),
          );
        }
      } catch (e) {
    ...
    test('creates new streams', () async {
      ...
      final stream2 = await withTimeout(
        streamReceived.future,
        'server stream acceptance',
      );
      ...
      expect((stream1).id(), equals((stream2).id()));
    ...
    test('handles concurrent streams', () async {
      ...
      try {
        ...
        for (var i = 0; i < streams1.length; i++) {
          final received1 = await withTimeout(
            streams1[i].read(),
            'stream${i + 1} read',
          );
          final received2 = await withTimeout(
            streams2[i].read(),
            'stream${i + 1} read',
          );
          print(
        ...
        print('Closing streams...');
        for (var i = 0; i < streams1.length; i++) {
          await streams1[i].close().catchError(
            (e) => print('Error closing client stream $i: $e'),
          );
          await streams2[i].close().catchError(
            (e) => print('Error closing server stream $i: $e'),
          );
        }
    ...
    test('handles session closure', () async {
      ...
      print('Creating streams...');
      final P2PStream<Uint8List> stream1 =
          await withTimeout(
                session1.openStream(core_context.Context()),
                'stream creation',
              )
    ...
    test('handles half-close (closeWrite)', () async {
      ...
      final receivedData = await withTimeout(
        stream1.read(),
        'stream1 read after closeWrite',
      );
      expect(
        receivedData,
        equals(testData),
    ...
    test('handles local read closure (closeRead)', () async {
      ...
      final readResult = await withTimeout(
        stream1.read(),
        'stream1 read after closeRead',
      );
      expect(
        readResult.isEmpty,
        isTrue,
    ...
    test(
      'handles large payload with rapid chunk delivery (OBP scenario)',
      () async {
        print('\n=== Large Payload with Rapid Chunk Delivery Test ===');
        print(
          'This test reproduces the scenario that causes OBP test failures',
        );
        ...
        try {
          ...
          print(
            'Starting rapid write operations (no delays between chunks)...',
          );
          ...
          while (receivedData.length < largeData.length) {
            try {
              ...
              if (readOperations % 10 == 0) {
                ...
                final progress =
                    (receivedData.length * 100 ~/ largeData.length);
                print(
