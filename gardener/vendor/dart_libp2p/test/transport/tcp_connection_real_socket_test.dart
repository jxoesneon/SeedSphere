...
void main() {
  ...
  group('TCPConnection with Real Sockets', () {
    ...
    setUp(() async {
      ...
      final remoteServerMultiaddr = MultiAddr(
        '/ip4/$localHostIp/tcp/$port',
      ); // Corrected: Constructor
    ...
    test('Server can send data and client receives it', () async {
      ...
      final receivedData = await clientConnection.read(
        testData.length,
      ); // Corrected: positional argument
    ...
    test(
      'Server sends multiple small chunks, client reads them sequentially',
      () async {
    ...
    test('Server sends partial data then closes, client reads available then EOF', () async {
      ...
      final chunkPartialSent = Uint8List.fromList([
        11,
        12,
        13,
        14,
        15,
      ]); // Server will send this
      ...
      final receivedPartial = await clientConnection.read(
        chunkPartialExpectedLength,
      );
      expect(
        receivedPartial,
