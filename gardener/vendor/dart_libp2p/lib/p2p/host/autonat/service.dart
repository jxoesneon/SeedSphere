...
class AutoNATService {
  ...
  Future<pb.Message_DialResponse> _handleDial(
    ...
    for (final maddrBytes in mpi.addrs) {
      ...
      try {
        final MultiAddr originalAddr = MultiAddr.fromBytes(
          Uint8List.fromList(maddrBytes),
        );
        MultiAddr addrToProcess = originalAddr;

        bool ipReplaced = false;
        final List<(mp_protocol.Protocol, String)> newComponents =
  ...
  Future<pb.Message_DialResponse> _doDial(AddrInfo pi) async {
    ...
    final ctx = Context(
      timeout: _config.dialTimeout,
    ); // Use context with timeout via constructor
    ...
    await dialer.peerstore.addrBook.addAddrs(
      pi.id,
      pi.addrs,
  ...
  void _startBackgroundTasks() {
    _throttleResetTimer = Timer.periodic(_config.throttleResetPeriod, (
      timer,
    ) async {
      await _mutex.lock(() async {
