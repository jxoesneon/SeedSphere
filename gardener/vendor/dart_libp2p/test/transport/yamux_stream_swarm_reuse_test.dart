...
void main() {
  ...
  group('YamuxStream Connection Re-use via Swarms', () {
    ...
    setUp(() {
      ...
      when(
        mockResourceManager.openStream(any, any),
      ).thenAnswer((_) async => mockStreamScope);
      when(mockResourceManager.viewPeer<PeerScope>(any, any)).thenAnswer((
        invocation,
      ) async {
        final callback =
            invocation.positionalArguments[1]
                as Future<PeerScope> Function(PeerScope);
    ...
    test(
      'swarm streams reuse yamux connections correctly',
      () async {
        ...
        final swarmA = await createTestSwarm(
          name: 'SwarmA',
          sharedTransport: mockTransport,
        );
        final swarmB = await createTestSwarm(
          name: 'SwarmB',
          sharedTransport: mockTransport,
        );
        ...
        try {
          ...
          await swarmB.peerstore.addrBook.addAddrs(swarmA.localPeer, [
            actualListenAddr,
          ], Duration(hours: 1));
          ...
          expect(
            sharedConn.isClosed,
            isFalse,
            reason: 'Connection should be alive before pause',
          );
          ...
          heartbeatTimer = Timer.periodic(Duration(milliseconds: 500), (timer) {
            heartbeatCount++;
            print(
              'Pause heartbeat $heartbeatCount: ${heartbeatCount * 500}ms elapsed',
            );
            ...
            if (heartbeatCount % 2 == 0) {
              ...
              print(
                '  - Active streams: ${streams.where((s) => !s.isClosed).length}',
              );
            }
          });
            ...
            await pauseCompleter.future.timeout(
              ...
              onTimeout: () {
                throw TimeoutException(
                  'Pause operation timed out after 5 seconds',
                  Duration(seconds: 5),
                );
              },
            );
          ...
          for (int i = 0; i < 2; i++) {
            ...
            final newStream = await swarmB.newStream(
              Context(),
              swarmA.localPeer,
            );
  ...
  config.muxers = [
    StreamMuxer(
      ...
      muxerFactory: (conn, isClient) {
        ...
        return YamuxSession(conn, const MultiplexerConfig(), isClient);
      },
    ),
  ];
...
void setupTransportDialing(
  ...
  when(mockTransport.dial(targetAddr)).thenAnswer((_) async {
    ...
    final (
      clientConn,
      serverConn,
    ) = EnhancedYamuxTransportConn.createConnectedPair(
      peer1: localPeer,
      peer2: targetPeer,
      ...
      enableFrameLogging:
          true, // Enable logging to see Yamux frame communication
    );

    print(
      'Created enhanced Yamux connection pair: ${clientConn.id} ↔ ${serverConn.id}',
    );
    print(
