...
void main() {
  group('NoiseXXPattern', () {
        ...
        throwsA(
          predicate(
            (e) =>
                e is StateError &&
                e.toString().contains(
                  'Message too short to contain ephemeral key',
                ),
          ),
        ),
      );
    ...
    test(
      'fails if remote static key is accessed before handshake completion',
      () {
        expect(
    ...
    test(
      'fails if trying to write message after handshake completion',
      () async {
        // Complete handshake
        final message1 = await initiator.writeMessage(Uint8List(0));
        await responder.readMessage(message1);
    ...
    test(
      'verifies different static keys produce different shared secrets',
      () async {
        final algorithm = X25519();
    ...
    test('verifies ephemeral keys are unique per handshake', () async {
      ...
      final message1 = await initiator.writeMessage(Uint8List(0));
      final ephemeral1 = message1.sublist(
        0,
        32,
      ); // First 32 bytes are ephemeral key
    ...
    test('handles error state transitions correctly', () async {
      ...
      expect(() => pattern.readMessage(msg1), throwsA(isA<StateError>()));
      expect(
        () => pattern.writeMessage(Uint8List(0)),
        throwsA(isA<StateError>()),
      );
    ...
    test('minimal hmac test', () async {
      ...
      final k1_init = await hmac.calculateMac([
        0x01,
      ], secretKey: initiator.sendKey);
      final k1_resp = await hmac.calculateMac([
        0x01,
      ], secretKey: responder.recvKey);
      expect(
        k1_init.bytes,
        equals(k1_resp.bytes),
    ...
    test(
      'proves payload is encrypted in final message with large payload',
      () async {
        // Complete first two messages
        final message1 = await initiator.writeMessage(Uint8List(0));
        await responder.readMessage(message1);
    ...
    test(
      'readMessage correctly parses final message with static key and MAC',
      () async {
    ...
    test('rejects messages with insufficient data', () async {
      ...
      final shortMessage = Uint8List(
        34,
      ); // Just enough for length prefix + ephemeral key
      shortMessage[0] = 0x00;
    ...
    test('proves handshake payload handling issue', () async {
      ...
      final encryptedTestData = await _encryptWithKey(
        testData,
        initiator.sendKey,
      );
      final decryptedTestData = await _decryptWithKey(
        encryptedTestData,
        responder.recvKey,
      );
    ...
    test(
      ...
      () async {
        ...
        // First 32 bytes should be responder's ephemeral key
        final responderEphemeralPub = await responderEphemeral
            .extractPublicKey();
        final responderEphemeralBytes = await responderEphemeralPub.bytes;
        expect(
          message2.sublist(0, 32),
    ...
    test(
      'Step 3: -> s, se - initiator sends static key and performs se',
      () async {
        // Create pattern instances with known keys for deterministic testing
        final algorithm = X25519();
        final initiatorStatic = await algorithm.newKeyPair();
        ...
        final message1 = await initiator.writeMessage(Uint8List(0));
        // Verify first message contains exactly the ephemeral key
        final initiatorEphemeralPub = await initiatorEphemeral
            .extractPublicKey();
        final initiatorEphemeralBytes = await initiatorEphemeralPub.bytes;
        expect(
          message1,
    ...
    test('validates static key access according to Noise XX pattern', () async {
      ...
      expect(() => initiator.remoteStaticKey, throwsA(isA<StateError>()));
      expect(() => responder.remoteStaticKey, throwsA(isA<StateError>()));
