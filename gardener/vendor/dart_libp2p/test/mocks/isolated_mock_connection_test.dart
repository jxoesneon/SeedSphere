...
void main() {
  group('MockConnection', () {
    ...
    test('proves read chunking issue', () async {
      ...
      // - 16 bytes of MAC
      final message =
          Uint8List(50) // 2 + 32 + 16
            ..[0] =
                0 // Length prefix high byte (48 in big endian)
    ...
    test('correctly handles large messages with length prefix', () async {
      ...
      final fullMessage =
          Uint8List(166) // 2 bytes length + 164 bytes data
            ..[0] =
                164 >>
                8 // High byte of length
    ...
    test('proves double buffering issue', () async {
      ...
      try {
        ...
        final fullMessage =
            Uint8List(6) // 2 bytes length + 4 bytes data
              ..[0] =
                  0 // Length prefix high byte (4 in big endian)
              ..[1] =
                  4 // Length prefix low byte
              ..setAll(2, message);
    ...
    test('handles noise protocol message sequence', () async {
      ...
      final message1 =
          Uint8List(34) // 2 bytes length + 32 bytes data
            ..[0] =
                0 // Length prefix high byte (32)
            ..[1] = 32; // Length prefix low byte
      message1.fillRange(2, 34, 0x42); // Fill with 0x42

      // Second message: e, ee, s, es (80 bytes)
      final message2 =
          Uint8List(82) // 2 bytes length + 80 bytes data
            ..[0] =
                0 // Length prefix high byte (80)
            ..[1] = 80; // Length prefix low byte
      message2.fillRange(2, 82, 0x43); // Fill with 0x43

      // Third message: s, se (48 bytes)
      final message3 =
          Uint8List(50) // 2 bytes length + 48 bytes data
            ..[0] =
                0 // Length prefix high byte (48)
            ..[1] = 48; // Length prefix low byte
