...
async function probe(timeoutMs = 3000) {
  for (const base of MIRRORS) {
    try {
      ...
      // Consider any HTTP status as reachable (handles Cloudflare 5xx and blocks)
...
async function fetchStreams(type, id, timeoutMs = 6000) {
  ...
  try {
    const url = `${DEFAULT_BASE}/all/torrents/${q}.html`
    ...
    // Follow first few detail links
    const detailLinks = Array.from(new Set(Array.from(html.matchAll(/href=\"(\/tor\/\d+\/.+?\.html)\"/gi)).map(m => m[1]))).slice(0, 6)
