...
void main() {
  ...
  group('Simplified Sequential Stream Test (TCP -> Noise -> Yamux)', () {
    ...
    test('Open, use, and close first stream, then open and use second stream', () async {
      ...
      final clientStream1Future = upgradedClientConn!.newStream(
        Context(),
      ...
      final P2PStream clientStream1 = await clientStream1Future.timeout(
        Duration(seconds: 5),
      ); // Explicit type P2PStream
      testLog.info(
        'Client stream 1 opened. ID (Yamux): \${(clientStream1 as YamuxStream).id()}',
      ...
      testLog.info(
        'Server stream 1 accepted. ID (Yamux): \${(serverStream1 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      ...
      final List<int> remainingFromServer1_list = await serverStream1.read(
        1,
      ); // Try to read 1 byte for EOF check
      final Uint8List remainingFromServer1 = Uint8List.fromList(
        remainingFromServer1_list,
      );
      expect(
      ...
      final P2PStream clientStream2 = await clientStream2Future.timeout(
        Duration(seconds: 5),
      ); // Explicit type P2PStream
      testLog.info(
        'Client stream 2 opened. ID (Yamux): \${(clientStream2 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
      final core_mux.MuxedStream serverStream2 = await serverStream2Future
          .timeout(Duration(seconds: 5)); // Explicit type
      testLog.info(
        'Server stream 2 accepted. ID (Yamux): \${(serverStream2 as YamuxStream).id()}',
      ); // Cast to YamuxStream for id()
