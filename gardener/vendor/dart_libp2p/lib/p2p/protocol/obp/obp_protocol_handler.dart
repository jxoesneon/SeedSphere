...
class OBPProtocolHandler {
  ...
  static Future<bool> _performClientHandshake(
    ...
    final handshakeAck = await _readFrame(
      stream,
      timeout: timeout,
      context: context,
    );
    ...
    try {
      ...
      final serverCapabilities = List<String>.from(
        ackData['capabilities'] as List,
      );
      _logger.info(
      ...
      _logger.warning(
        '[$context] Failed to parse handshake acknowledgment: $e',
      );
      return false;
    }
  }
  ...
  static Future<bool> _performServerHandshake(
    ...
    final handshakeReq = await _readFrame(
      stream,
      timeout: timeout,
      context: context,
    );
    ...
    try {
      ...
      final clientCapabilities = List<String>.from(
        reqData['capabilities'] as List,
      );
      _logger.info(
  ...
  static Future<OBPFrame?> _readFrame(
    ...
    try {
      ...
      final payloadLength = headerBuffer.getUint32(
        8,
        Endian.big,
      ...
      _logger.fine(
        '[$context] Read frame: ${frame.type} (${frame.length} bytes)',
      );
      return frame;
    } catch (e, stackTrace) {
  ...
  static Future<void> _writeFrame(
    ...
    try {
      ...
      _logger.info(
        '[$context] WRITE_VERIFY: About to write ${frame.type} frame',
      );
      _logger.info(
  ...
  static Future<Uint8List?> _readExactBytes(
      ...
      try {
        ...
        if (chunk.isEmpty) {
          ...
          if (streamBuffer.isEmpty) {
            _logger.info(
              '[$context] READ_VERIFY: Clean EOF - no data received',
            );
    ...
    final result = Uint8List.fromList(
      streamBuffer.take(expectedBytes).toList(),
    );
