  ...
  test(
    ...
    () async {
      ...
      try {
        ...
        final stream1Future = session1.openStream(
          core_context.Context(),
        ); // Changed to openStream
        final stream2Future = session2.acceptStream();

        // Add timeout to stream creation
        final stream1 =
            await stream1Future.timeout(
                  Duration(seconds: 5),
        ...
        await stream1.close().timeout(
          Duration(seconds: 5),
          onTimeout: () =>
              throw TimeoutException('Timeout waiting for client stream close'),
        );
        ...
        await stream2.close().timeout(
          Duration(seconds: 5),
          onTimeout: () =>
              throw TimeoutException('Timeout waiting for server stream close'),
        );
