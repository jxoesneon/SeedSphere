...
class SecuredConnection implements TransportConn {
  ...
  SecuredConnection(
    ...
    _encryptionKey.extractBytes().then(
      (bytes) => _log.finer('  - _encryptionKey.bytes: $bytes'),
    );
    _log.finer('  - _decryptionKey.hashCode: ${_decryptionKey.hashCode}');
    _decryptionKey.extractBytes().then(
      (bytes) => _log.finer('  - _decryptionKey.bytes: $bytes'),
    );
  }
  ...
  Future<Uint8List> _readAndDecryptMessage() async {
    ...
    try {
      final plaintext = await algorithm.decrypt(
        crypto.SecretBox(encrypted, nonce: nonce, mac: crypto.Mac(mac)),
