...
void main() {
  ...
  group('UDX Transport: Multiple Clients from Same IP, Different Ports', () {
    ...
    setUpAll(() async {
      ...
      client2KeyPair = await crypto_ed25519
          .generateEd25519KeyPair(); // Re-enable client2

      serverPeerId = await core_peer_id_lib.PeerId.fromPublicKey(
        serverKeyPair.publicKey,
      );
      client1PeerId = await core_peer_id_lib.PeerId.fromPublicKey(
        client1KeyPair.publicKey,
      );
      ...
      Future<BasicHost> createHost(
        ...
        final swarmConfig = p2p_config.Config()
          ...
          ..connManager = connManager
          ..eventBus =
              BasicBus() // Swarm's own event bus
      ...
      serverHost = await createHost(serverKeyPair, serverPeerId, [
        initialListen,
      ]);
      client1Host = await createHost(client1KeyPair, client1PeerId);
      client2Host = await createHost(
        client2KeyPair,
        client2PeerId,
      ); // Re-enable client2
      ...
      client1Host.peerStore.addrBook.addAddrs(serverPeerId, [
        serverListenAddr,
      ], AddressTTL.permanentAddrTTL);
      client1Host.peerStore.keyBook.addPubKey(
        serverPeerId,
        serverKeyPair.publicKey,
      );
      client2Host.peerStore.addrBook.addAddrs(serverPeerId, [
        serverListenAddr,
    ...
    test(
      ...
      () async {
        ...
        serverHost.setStreamHandler(testProtocolID, (stream, remotePeer) async {
          ...
          try {
            final receivedData = await stream.read().timeout(
              Duration(seconds: 10),
            );
            print(
            ...
            if (remotePeer == client2PeerId &&
              ...
              serverReceivedDataFromClient2.completeError(
                e,
              ); // Re-enable client2
          } finally {
        ...
        final connsToClient1List = serverHost.network.connsToPeer(
          client1PeerId,
        );
        final connToClient1 = connsToClient1List.isNotEmpty
            ? connsToClient1List.first
            : null;
        final connsToClient2List = serverHost.network.connsToPeer(
          client2PeerId,
        ); // Re-enable client2
        ...
        final random = Random();
        final dataC1 = Uint8List.fromList(
        ...
        final echoedToC1 = await client1Stream.read().timeout(
          Duration(seconds: 10),
        );
        expect(
          echoedToC1,
        ...
        final echoedToC2 = await client2Stream.read().timeout(
          Duration(seconds: 10),
        );
        expect(
          echoedToC2,
