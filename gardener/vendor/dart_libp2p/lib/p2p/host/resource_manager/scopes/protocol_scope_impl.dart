...
class ProtocolScopeImpl extends ResourceScopeImpl implements ProtocolScope {
  ...
  ResourceScopeImpl getPeerSubScope(
    ...
    print(
      'DEBUG: ProtocolScopeImpl ($name) getPeerSubScope for $peerId. Parent streams: In=${parentStat.numStreamsInbound}, Out=${parentStat.numStreamsOutbound}.',
    );

    return _peerSubScopes.putIfAbsent(peerId, () {
      print(
        'DEBUG: ProtocolScopeImpl ($name) creating new peer sub-scope for $peerId.',
      );
      final peerProtocolLimit = limiter.getProtocolPeerLimits(
        this.protocol,
        peerId,
      );
      final scopeName =
