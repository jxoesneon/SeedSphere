...
void main() {
  group('SwarmStream and UDXP2PStreamAdapter Interaction', () {
    ...
    setUpAll(() async {
      ...
      final clientTransport = UDXTransport(
        connManager: connManager,
        udxInstance: udxInstance,
      );
      final serverTransport = UDXTransport(
        connManager: connManager,
        udxInstance: udxInstance,
      );
      ...
      clientSwarm.peerstore.addrBook.addAddrs(serverPeerId, [
        serverListenAddr,
    ...
    Future<
      (core_network_stream.P2PStream, core_network_stream.P2PStream, Conn, Conn)
    >
    createStreamsWithConnections() async {
      ...
      final clientConn = await clientSwarm.dialPeer(
      ...
      final serverStream = await serverStreamFuture.timeout(
        Duration(seconds: 5),
      );
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          final serverReadFuture = serverP2PStream.read().timeout(
            Duration(seconds: 10),
          );
  ...
  group('Stream EOF and Best Practices Demonstration', () {
    ...
    test('Chunked Data Reading with EOF', () async {
      ...
      try {
        ...
        final expectedTotal = Uint8List.fromList(
    ...
    test('Protocol-Style Reading (Length-Prefixed)', () async {
      ...
      try {
        ...
        final ack = await StreamEOFUtils.safeRead(
          clientStream,
          context: 'Client-ACK',
        );
    ...
    test('Bidirectional Communication with Proper EOF', () async {
      ...
      try {
        final clientMessage = Uint8List.fromList(
          'Hello from client!'.codeUnits,
        );
        final serverMessage = Uint8List.fromList(
          'Hello from server!'.codeUnits,
        );

        // Both sides communicate simultaneously
        final serverLogic = () async {
          print('[Server] Reading client message...');
          final receivedFromClient = await StreamEOFUtils.safeRead(
            serverStream,
            context: 'Server',
          );
        ...
        final clientLogic = () async {
          ...
          final receivedFromServer = await StreamEOFUtils.safeRead(
            clientStream,
            context: 'Client',
          );
