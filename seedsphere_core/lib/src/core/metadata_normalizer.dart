...
class MetadataNormalizer {
  ...
  /// Standardizes resolution string: '4K', '1080p', '720p', or 'SD'.
  static String mapQuality(String q) => _extractResolution(q);

  /// Extracts the release year from a title string.
  ...
  /// Cleans a title to its "natural" form (removes year, resolution, etc.).
  static String toTitleNatural(String title) {
    var out = title;
    // Remove year
    out = out.replaceAll(RegExp(r'\(?\b(19|20)\d{2}\b\)?'), '');
    // Remove brackets content
    out = out.replaceAll(RegExp(r'\[.*?\]'), '');
    // Remove common tags
    out = out.replaceAll(
      RegExp(
        r'\b(4k|2160p|1080p|720p|480p|fhd|uhd|hdtv|web-dl|bluray|x264|x265|hevc|h264|h265|extended|remastered)\b',
        caseSensitive: false,
      ),
      '',
    );
