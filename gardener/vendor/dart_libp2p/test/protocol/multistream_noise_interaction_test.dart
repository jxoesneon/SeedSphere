...
class MockConnStats implements ConnStats {
  ...
  MockConnStats({Stats? stats, this.numStreams = 0})
    : stats =
          stats ?? Stats(direction: Direction.inbound, opened: DateTime.now());
}
...
class MockP2PStream implements P2PStream<Uint8List> {
  ...
  Future<Uint8List> read([int? count]) async {
    if (_readBuffer.isNotEmpty) {
      ...
      final bytesToRead = (count != null && count < available && count > 0)
          ? count
          : available;
      if (bytesToRead > 0) {
        final result = Uint8List.fromList(
          _readBuffer.toBytes().sublist(0, bytesToRead),
        );
        final remainingBytes = Uint8List.fromList(
          _readBuffer.toBytes().sublist(bytesToRead),
        );
        _readBuffer.clear();
  ...
  Future<void> closeRead() async {
    ...
    if (_pendingReadCompleter != null &&
        ...
        _readBuffer.isEmpty) {
      _pendingReadCompleter!.completeError(
        StateError("Stream closed for reading"),
      );
      _pendingReadCompleter = null;
    }
  }
...
void main() {
  group('MultistreamMuxer with Noise Handshake', () {
    ...
    setUp(() async {
      ...
      clientPeerId = await core_peer.PeerId.fromPublicKey(
      ...
      serverPeerId = await core_peer.PeerId.fromPublicKey(
      ...
      () async {
        ...
        serverMuxer.addHandler(serverNoiseSec.protocolId, (
          ...
          try {
            ...
            final serverSecuredConn = await serverNoiseSec.secureInbound(
              adapterFromServer,
            );
        ...
        final serverHandlingFuture = serverMuxer
            .handle(serverUnsecuredStream)
            .catchError((e) {
        ...
        final SecuredConnection clientSecuredConn = await clientNoiseSec
            .secureOutbound(adapterFromClient);
        ...
        final testMessage = Uint8List.fromList(
          utf8.encode('hello noisy multistream world!'),
        );

        // Client writes, Server reads
        await clientSecuredConn.write(testMessage);
        final receivedByServer = await serverSecuredConn.read(
          testMessage.length,
        );
        expect(
          receivedByServer,
        ...
        final testMessageFromServer = Uint8List.fromList(
          utf8.encode('greetings from server!'),
        );
        await serverSecuredConn.write(testMessageFromServer);
        final receivedByClient = await clientSecuredConn.read(
          testMessageFromServer.length,
        );
        expect(
          receivedByClient,
