...
class UDXTransport implements Transport {
  ...
  Future<TransportConn> _performDial(
    ...
    try {
      ...
      _logger.fine(
        '[UDXTransport._performDial] Handshake start for $host:$port',
      );
...
class UDXSessionConn implements MuxedConn, TransportConn {
  ...
  UDXSessionConn({
          ...
          .listen(
            (event) {
              ...
              final classifiedException =
                  UDXExceptionHandler.classifyUDXException(
                    error,
    ...
    _logger.fine(
      '[UDXSessionConn $id] Subscribing to initialStream closeEvents.',
    );
    ...
    if (_isDialer) {
      ...
      _socketMessageSubscription = _udpSocket
          .on('unmatchedUDXPacket')
          .listen(
  ...
  void _handleDialerUnmatchedPacket(dynamic event) {
    ...
    final expectedRemotePort = int.parse(
      _remoteMultiaddr.valueForProtocol('udp')!,
    );
  ...
  Future<MuxedStream> openStream(Context context) async {
    ...
    final remoteHost =
        _remoteMultiaddr.valueForProtocol('ip4') ??
        _remoteMultiaddr.valueForProtocol('ip6');
    final remotePort = int.parse(
      _remoteMultiaddr.valueForProtocol('udp') ?? '0',
    );
  ...
  Future<void> close() async {
    ...
    _activeStreams.values.toList().forEach((adapter) {
      _logger.fine(
        '[UDXSessionConn $id] Closing stream adapter ${adapter.id()}',
      );
  ...
  Future<void> closeWithError(dynamic error, [StackTrace? stackTrace]) async {
    _logger.fine(
      '[UDXSessionConn $id] closeWithError called with error: $error',
    );
...
class _UDXConnStatsImpl extends ConnStats {
  _UDXConnStatsImpl({required super.stats, required super.numStreams});
}
