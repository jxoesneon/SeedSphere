...
class IdentifySnapshot {
  ...
  Future<bool> equals(IdentifySnapshot other) async {
    ...
    final otherRecord = other.record is Future
        ? await other.record
        : other.record;
...
class IdentifyService implements IDService {
  ...
  IdentifyService(this.host, {IdentifyOptions? options})
  ...
  Future<void> start() async {
    ...
    _evtPeerProtocolsUpdated = await host.eventBus.emitter(
      EvtPeerProtocolsUpdated,
    );
    _evtPeerIdentificationCompleted = await host.eventBus.emitter(
      EvtPeerIdentificationCompleted,
    );
    _evtPeerIdentificationFailed = await host.eventBus.emitter(
  ...
  Future<P2PStream?> _newStreamAndNegotiate(
    ...
    try {
      ...
      final selectedProtocol = await protocolMuxer.selectOneOf(stream, [
        protoIDString,
      ]);
  ...
  Future<void> sendIdentifyResp(P2PStream stream, bool isPush) async {
    ...
    try {
      ...
      final snapshot = await _currentSnapshotMutex.synchronized(
        () => _currentSnapshot,
      );
      _log.fine(
      ...
      final signedRecordBytes = await _getSignedRecord(
        snapshot,
  ...
  Future<Uint8List?> _getSignedRecord(IdentifySnapshot snapshot) async {
    ...
    final actualRecord = snapshot.record is Future
  ...
  Future<void> handleIdentifyResponse(P2PStream stream, bool isPush) async {
    ...
    try {
      ...
      await _connsMutex.synchronized(() async {
        ...
        final sup = await host.peerStore.protoBook.supportsProtocols(
          conn.remotePeer,
          [idPush],
        );
        if (sup.isNotEmpty) {
  ...
  Future<void> _consumeMessage(Identify mes, Conn conn, bool isPush) async {
    ...
    final mesProtocols = mes.protocols
        .map((s) => s)
        .toList(); // Already List<String>
    _log.finer(
    ...
    if (isPush) {
      ...
      _evtPeerProtocolsUpdated.emit(
        EvtPeerProtocolsUpdated(peer: p, added: added, removed: removed),
      );
    }
    ...
    await host.peerStore.addrBook.updateAddrs(
      p,
    ...
    if (signedPeerRecord != null) {
      try {
        ...
        final signedAddrs = await _consumeSignedPeerRecord(
          conn.remotePeer,
          signedPeerRecord,
        );
  ...
  Future<void> _identifyConn(Conn conn) async {
    ...
    try {
      ...
      final streamNegotiationDuration = DateTime.now().difference(
        identifyStart,
      );
      if (stream == null) {
        _log.severe(
...
class _NetNotifiee implements Notifiee {
  ...
  Future<void> disconnected(Network network, Conn conn) async {
    ...
    await _ids.host.peerStore.addrBook.updateAddrs(
      peerId,
      ConnectedAddrTTL,
      TempAddrTTL,
    );
    await _ids.host.peerStore.addrBook.addAddrs(
      peerId,
      addrs.sublist(0, n),
      RecentlyConnectedAddrTTL,
    );
