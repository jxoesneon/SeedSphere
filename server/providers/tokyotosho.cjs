...
async function fetchStreams(type, id, timeoutMs = 6000) {
  ...
  try {
    const url = `${DEFAULT_BASE}/search.php?terms=${q}&type=1` // type=1: anime
    ...
    const detailLinks = Array.from(new Set(Array.from(html.matchAll(/href=\"(\/torrent\/[^"]+)\"/gi)).map(m => m[1]))).slice(0, 8)
