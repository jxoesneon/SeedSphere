...
void main() {
  ...
  group('UDXTransport with Noise and OBP Integration Test', () {
    ...
    test(
      ...
      () async {
        ...
        final clientDialFuture = clientTransport.dial(actualListenAddr).then((
          conn,
        ) {
          clientRawConn = conn;
        ...
        final List<Conn> upgradedConns = await Future.wait([
          clientUpgradedFuture,
          serverUpgradedFuture,
        ]);
        ...
        clientStream =
            await (clientUpgradedConn as core_mux_types.MuxedConn).openStream(
        ...
        final serverHandlerFuture = _handleBasicServerOBPStream(
          serverStream,
          'BasicTransportTest',
        );
  ...
  group('UDXTransport with Noise/OBP, Real ConnMgr and RsrcMgr', () {
    ...
    test(
      ...
      () async {
        ...
        final serverAcceptStreamFuture =
            (serverUpgradedConn as core_mux_types.MuxedConn).acceptStream().then((
              stream,
            ) {
              serverStream = stream as YamuxStream;
              print(
  ...
  group('Swarm with UDX, Noise, and OBP', () {
    ...
    setUpAll(() async {
      ...
      clientHost.peerStore.addrBook.addAddrs(serverPeerId, [
        serverListenAddr,
    ...
    test(
      ...
      () async {
        ...
        try {
          ...
          clientStream = await clientHost.newStream(serverPeerId, [
...
Future<void> _testOBPFeatures(
  ...
  currentStream = await ensureHealthyStream(
    currentStream,
    'Test 1 - Pre-check',
  );
  ...
  try {
    final mediumPayload = Uint8List.fromList(
      List.generate(1024 * 32, (i) => i % 256),
    ); // 32KB
  ...
  try {
    final largePayload = Uint8List.fromList(
      List.generate(1024 * 100, (i) => i % 256),
    ); // 100KB
