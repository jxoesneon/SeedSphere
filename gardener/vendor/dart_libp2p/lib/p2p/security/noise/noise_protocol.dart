...
class NoiseSecurity implements SecurityProtocol {
  ...
  Future<SecuredConnection> secureOutbound(TransportConn connection) async {
    ...
    try {
      ...
      final signature = await _identityKey.privateKey.sign(
        pattern.remoteStaticKey,
      );
      ...
      final responderEncryptedPayloadBytes = await _readLibp2pHandshakePayload(
        tempSecuredConn,
      );
  ...
  Future<SecuredConnection> secureInbound(TransportConn connection) async {
    ...
    try {
      ...
      final initiatorEncryptedPayloadBytes = await _readLibp2pHandshakePayload(
        tempSecuredConn,
      );
