...
class MockResourceScope extends ResourceScopeImpl {
  ...
  MockResourceScope(super.limit, super.name, {super.owner, super.edges});
...
void main() {
  ...
  group('ResourceScopeImpl Reference Counting', () {
    ...
    test(
      'isUnused() returns true if done, regardless of refCnt or resources',
      () {
        final limit = createTestLimit();
        final scope = ResourceScopeImpl(limit, 'testScope');
  ...
  group('ResourceScopeImpl DAG Hierarchy (Parent-Child)', () {
    ...
    test(
      'child.removeStream() propagates to parent via _removeStreamForChild',
      () {
        expect(() => childScope.addStream(Direction.outbound), returnsNormally);
        expect(childScope.stat.numStreamsOutbound, 1);
        expect(mockParent.stat.numStreamsOutbound, 1);
    ...
    test(
      ...
      () {
        ...
        final mockGrandParent = MockResourceScope(
          grandParentLimit,
          'mockGrandParent',
        );
        ...
        expect(
          () => actualChild.addStream(Direction.outbound),
          returnsNormally,
        );
  ...
  group('ResourceScopeImpl Span Hierarchy (Owner-Span)', () {
    ...
    test('beginSpan() increments owner refCnt and creates valid span', () async {
      // Made async
    ...
    test(
      'span.done() calls _releaseResourcesForChild and decRef on owner',
      () async {
        expect(
          () => spanScope.addStream(Direction.outbound),
          returnsNormally,
  ...
  group('ResourceScopeImpl Resource Limits and Errors', () {
    ...
    test(
      'addStream() throws ResourceScopeClosedException if scope is done',
      () {
        final limit = createTestLimit();
        final scope = ResourceScopeImpl(limit, 'testScope');
        scope.done();
    ...
    test(
      ...
      () async {
        ...
        expect(
          () async => await scope.reserveMemory(30, 0),
          throwsA(isA<network_errors.ResourceLimitExceededException>()),
        );
        expect(
          scope.stat.memory,
