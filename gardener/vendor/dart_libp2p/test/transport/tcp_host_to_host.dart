  ...
  */
  ...
  group('Host-to-Host TCP, Noise, Yamux, Ping (via BasicHost)', () {
    ...
    setUpAll(() async {
      ...
      clientPeerId = await core_peer_id_lib.PeerId.fromPublicKey(
      ...
      final initialListen = MultiAddr(
        '/ip4/127.0.0.1/tcp/0',
      ); // Changed to TCP listen address
      ...
      final clientHostConfig = p2p_config.Config()
        ...
        ..connManager =
            connManager // Shared connManager
        ..addrsFactory =
            passThroughAddrsFactory // For BasicHost.addrs getter
        ..negotiationTimeout =
      ...
      final serverHostConfig = p2p_config.Config()
        ...
        ..listenAddrs =
            [initialListen] // For BasicHost to know its intended listen addrs
        ..muxers = muxerDefs
      ...
      serverListenAddr = serverHost.addrs.firstWhere(
        ...
        orElse: () =>
            throw StateError("No TCP listen address found for server host"),
      ); // Changed to TCP
    ...
    test(
      ...
      () async {
        ...
        final Completer<core_network_stream.P2PStream> serverStreamCompleter =
            Completer();
        final Completer<void> serverHandlerFinishedProcessing = Completer();
        ...
        serverHost.setStreamHandler(testProtocolID, (stream, remotePeer) async {
          ...
          if (!serverStreamCompleter.isCompleted) {
            ...
            if (!serverHandlerFinishedProcessing.isCompleted)
              serverHandlerFinishedProcessing.completeError(
                StateError("Duplicate stream"),
              );
            return;
          }
